...
if(f2) # 1/f^2 weighting
  ...
  for(m = 1:bands) # passband only
    ...
    if(bandTest(m)) # apply 1/f^2 weighting in the passband, only
      ...
      if(m == 1) # take care of F(1)=0
        ## It turns out there's no difference with/without tmp for F(1)=0, so
        ## just consider the singularities for Ci(x) and cos(x)/x as zero. This
        ## way numerical problems for large numbers are avoided in the case of
        ## e.g. Ci(1e-6), or cos(1e-6)/1e-6.
        sc += -K(m)*intercept*pi*n;
      else
        tmp = expint(1.0i*n*w(m));
